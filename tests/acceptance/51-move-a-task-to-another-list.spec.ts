...
// Moving a task to another list (#51). The UI these tests drive: every task row carries a "Move <title>" button beside
// its Edit and Delete ones (the reorder buttons keep their own "Move <title> up" / "Move <title> down" names, which
// exact: true tells apart from it). It opens a "Move <title>" form holding a "Move to list" picker of the user's other
// lists and a "Move task" button, and once the server confirms, the source list's page shows a notice naming the
// destination list, the way a deletion shows "Deleted …".
//
...
// read-only, so a user owning a single list is a state these tests cannot reach through the UI.
...
test.describe('move a task to another list', () => {
  ...
  test('AC4: a confirmed move takes the task off the source list and puts it on the destination list', async ({ page }) => {
    ...
    await page.reload()
    // The source list being empty also proves its tasks have loaded before asserting the moved task is gone.
