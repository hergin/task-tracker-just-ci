...
import { dueState, isDateKey, nextDateKey, toDateKey } from '../../src/lib/dates'
...
describe('nextDateKey', () => {
  // The day a task pushed from Today lands on: the calendar day after `today`, across months, years and a leap day.
  it.each([
  ...
  ] as const)('the day after %s is %s', (date, expected) => {
    expect(nextDateKey(date)).toBe(expected)
  })
})
